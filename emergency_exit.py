    ...
    try:
        for pair in matched_pairs:
            ...
            if tasks:
                ...
                # `isinstance(res, Exception)` misses Lighter's None/False failure
                # returns. In an emergency-exit tool that meant printing
                # "Close orders sent" for a close that was never accepted.
                close_results = [
                ...
                if rejected:
                    ...
                    print(f"  {Colors.GREEN}✓ Close orders accepted for {symbol} "
                          f"(closure verified below){Colors.RESET}")
        ...
        for pair in matched_pairs:
            ...
            # Check Lighter
            l_market_id, l_price_tick, l_amount_tick = await lighter_client.get_lighter_market_details(order_api, symbol_clean)
            # An unreadable position must never be reported as closed. This is the
            # tool you reach for in a crisis; a false "fully closed" here is worse
            # than no tool at all.
            try:
                ...
                print(f"  {Colors.RED}✗ {symbol}: CANNOT VERIFY Lighter position ({e}){Colors.RESET}")
                print(f"    {Colors.YELLOW}Close was attempted but its effect is UNKNOWN. "
                      f"Check Lighter manually before assuming this is flat.{Colors.RESET}")
                continue
