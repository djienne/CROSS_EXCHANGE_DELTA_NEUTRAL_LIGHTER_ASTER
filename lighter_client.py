...
class PositionFetchError(Exception):
    ...
    Exists so that "I could not read the position" is distinguishable from
    "the position is zero". Conflating those is a money-losing bug: see the
    note in get_lighter_open_size.
    """
    pass
...
async def get_lighter_open_size(account_api, account_index: int, market_id: int, symbol: Optional[str] = None) -> float:
    ...
    try:
        ...
        #
        # This used to fall back to 0.0 with the comment "avoid preventing close
        # attempts". It achieved the exact opposite of its stated intent: 0.0 means
        # "no position", so close_delta_neutral_position saw nothing to close, sent
        # no order, re-checked, saw 0.0 again, and printed "Hedge closed
        # successfully on both exchanges" -- while the Lighter leg was still open.
        # The cycle was then marked successful and current_position cleared, so the
        # live leg was forgotten permanently, with no stop-loss and no monitoring.
        #
        # A failed read is not evidence of a flat position. Raise, and let callers
        # decide: retry, halt, or refuse to report success.
        logger.error(f"Lighter: Failed to get account position size due to an error: {e}", exc_info=True)
        raise PositionFetchError(
            f"Lighter position read failed for {market_label}: {e}"
        ) from e
...
async def get_lighter_position_pnl(account_api, account_index: int, market_id: int) -> float:
    ...
    try:
        ...
        # Same rule as get_lighter_open_size: an unreadable PnL is NOT a PnL of 0.
        #
        # This mattered specifically for the stop-loss. The monitor computes
        # `worst_pnl = min(lighter_pnl, aster_pnl)`, so a swallowed 0.0 meant a
        # Lighter leg deep underwater contributed nothing to the worst-leg figure
        # and the stop-loss could never fire on it -- the leg most likely to be
        # losing was the one silently excluded from the check.
        logger.error(f"Lighter: Failed to get PnL: {e}")
        raise PositionFetchError(
            f"Lighter PnL read failed for market {market_id}: {e}"
        ) from e
